...
class Consensus_D_F(gym.Env):
    ...
    def __init__(self, num_agents=3, num_iterations=10000, dt=0.001, max_neighbors=2):
        ...
        # num_iterations 和 dt 的设置决定了模拟的时间长度和精度
        # num_iterations 代表模拟总时间步数，dt 是每个时间步的长度，组合起来模拟连续时间过程
    ...
    class Agent:
        ...
        def update_position(self, t, dt, trigger):
            if trigger == 1:
                position_differences = [self.last_broadcast_position - neighbor.last_broadcast_position
                                        for neighbor in self.neighbors if self.is_neighbor(neighbor)]
                if all(abs(diff) < self.epsilon for diff in position_differences):
                    self.u_i = 0
                else:
                    self.u_i = -sum(position_differences)
