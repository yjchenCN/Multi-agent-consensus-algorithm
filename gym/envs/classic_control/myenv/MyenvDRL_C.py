...
class Consensus_C(gym.Env):
    ...
    def __init__(self, num_agents=5, num_iterations=200, dt=0.1):
        self.c0_min, self.c0_max = 0.0, 0.001
        self.c1_min, self.c1_max = 0.0, 8.0
        self.alpha_min, alpha_max = 0.0, 2.0
        # 动作空间定义为c0, c1, alpha组合的索引
        self.action_space = spaces.Box(low=np.array([self.c0_min, self.c1_min, self.alpha_min]),
                                high=np.array([self.c0_max, self.c1_max, alpha_max]),
                                dtype=np.float32)
    ...
    def step(self, action):
        ...
        c_0, c_1, alpha = action  # 使用单一索引从action_matrix中检索
